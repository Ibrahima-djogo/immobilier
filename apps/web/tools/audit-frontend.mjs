...
function resolveImportedFile(importerFile, specifier) {
...
for (const file of sourceFiles) {
  ...
  for (const token of mojibakeTokens) {
    if (content.includes(token)) {
      issues.push(
        `${relative}: encodage suspect détecté (${token})`,
      );
      break;
    }
  }
  ...
  for (const match of cssImports) {
    ...
    const cssPath = resolveImportedFile(file, specifier);

    if (cssPath && !fs.existsSync(cssPath)) {
      issues.push(
        `${relative}: CSS Module introuvable -> ${specifier}`,
      );
    }
  }
  ...
  const hrefs = [
    ...
    .filter(
      (href) =>
        !ignoredHrefPrefixes.some((prefix) =>
  ...
  for (const href of hrefs) {
    ...
    if (!exists) {
      warnings.push(
        `${relative}: lien local sans route détectée -> ${href}`,
      );
    }
  }
  ...
  for (const duplicate of new Set(duplicateEntries)) {
    issues.push(
      `${relative}: route dupliquée dans un tableau -> ${duplicate}`,
    );
  }
}
