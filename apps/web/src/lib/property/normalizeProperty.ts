...
function resolveCoordinates(
  ...
  if (nested && typeof nested === "object") {
    ...
    const lat = firstNumber(rec.lat, rec.latitude);
    const lng = firstNumber(rec.lng, rec.lon, rec.longitude);
    if (lat && lng) return { lat, lng };
    if (Number.isFinite(Number(rec.lat)) && Number.isFinite(Number(rec.lng))) {
      return { lat: Number(rec.lat), lng: Number(rec.lng) };
    }
  }
  const lat = firstNumber(raw.latitude, raw.lat);
  const lng = firstNumber(raw.longitude, raw.lng, raw.lon);
  if (lat && lng) return { lat, lng };
  if (
    raw.latitude != null &&
    raw.longitude != null &&
