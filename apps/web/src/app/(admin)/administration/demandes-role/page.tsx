...
export default function RoleRequestsPage() {
  ...
  const filtered = useMemo(
    () =>
      roleRequests.filter(
        (r) =>
          `${r.name} ${r.email} ${r.reference}`
            .toLowerCase()
            .includes(debouncedQuery.toLowerCase()) &&
          (status === "TOUS" || r.status === status) &&
          (role === "TOUS" || r.requestedRole === role),
      ),
  ...
  return (
    <AdminShell
      active="roles"
      eyebrow="Vérification des identités"
      title="Demandes de rôle"
      description="Examinez les demandes Propriétaire et Agence avec leurs justificatifs."
    >
      ...
      <p className={styles.resultCount}>
        <strong>{filtered.length}</strong> demande(s) affichée(s)
      </p>
        ...
        <section className={`${styles.card} ${styles.table}`}>
          ...
          {filtered.map((r) => (
            <article key={r.id}>
              ...
              <small>{r.documents} fichier(s)</small>
              <small>{r.submittedAt}</small>
              <Link href={`/administration/demandes-role/${r.id}`}>
                Examiner
