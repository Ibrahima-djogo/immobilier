...
export default function EditAgencyPropertyPage() {
  ...
  if (loading) {
    return (
      <AgencyShell
        active="biens"
        eyebrow="Modification professionnelle"
...
function EditAgencyPropertyForm({
  property,
  onUpdated,
}: {
  ...
  const [form, setForm] = useState({
    ...
    client: property.clientDisplayName || "",
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      const payload = buildPropertyApiPayload({
        ...
        clientDisplayName: form.client || null,
        clientReference: property.clientReference ?? null,
  ...
  return (
    <AgencyShell
      ...
      eyebrow="Modification professionnelle"
      title="Modifier le bien"
      description="Mettez à jour les informations et le mandat."
    >
      <Link href={`/agence/biens/${property.slug}`} className={styles.back}>
        <ArrowLeft size={15} />
        Retour à la fiche
      </Link>
      {saved ? (
        ...
        <div className={styles.grid}>
          ...
          <label>
            ...
            <input
              value={form.client}
              onChange={(e) =>
                setForm((v) => ({ ...v, client: e.target.value }))
              }
            />
          </label>
          <label>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((v) => ({ ...v, mandateType: e.target.value }))
              }
            >
