...
export default function EditAgencyAdPage() {
  ...
  return (
    <AgencyShell
      ...
      eyebrow="Modification"
      title="Modifier l’annonce"
      description="Modifiez le contenu avant une nouvelle soumission (Demo API)."
    >
      <Link href={routes.agencyAd(ad.id)} className={styles.back}>
        <ArrowLeft size={15} />
        Retour au détail
      </Link>
      {saved && (
      ...
      <form className={`${styles.card} ${styles.form}`} onSubmit={onSubmit}>
        ...
        <div>
          <p>
            Une modification sensible peut provoquer une nouvelle modération.
            Seule l’administration peut publier l’annonce.
          </p>
