...
import { requestBelongsToOwner } from "@/lib/verification-fonciere/display";
import {
...
export default function OwnerFonciereVerificationsPage() {
  ...
  const visible = useMemo(
    () => items.filter((item) => requestBelongsToOwner(item, DEMO_OWNER_ID)),
    [items],
  );
  ...
  return (
    <>
      <OwnerPageHeader
        ...
        description="Consultez les demandes de vérification foncière officielle concernant vos terrains."
      ...
      {!loaded ? (
        ...
        <section className={styles.empty}>
          ...
          <p>
            Aucune demande de vérification foncière n’a été reçue pour vos
            terrains.
          </p>
        </section>
      ) : (
          ...
          <section className={styles.list} aria-label="Demandes foncières">
            {visible.map((request) => (
              <FonciereRequestCard
                key={request.id}
                request={request}
                href={routes.ownerFonciereVerification(request.id)}
