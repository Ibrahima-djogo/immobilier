...
export function PropertyLocationMap({
  ...
  async function runSearch() {
    ...
    if (!trimmed) {
      setError("Saisissez un lieu, une rue ou un repère à rechercher.");
      return;
    }
    setError(null);
    setResults([]);
    ...
    try {
      ...
      if (found.length === 0) {
        setError(
          "Aucun lieu correspondant trouvé. Essayez un autre nom ou ajoutez un repère.",
        );
      }
    } catch (err) {
      setError(
      ...
      {!readOnly ? (
        <div className={styles.fields}>
          ...
          <label>
            Quartier
            <input
              list="property-quarter-suggestions"
              value={value.quarter}
              onChange={(e) => patch({ quarter: e.target.value })}
              placeholder="Ex. Kipé, Kaporo Rails…"
              autoComplete="off"
            />
            <datalist id="property-quarter-suggestions">
              {quarters.map((quarter) => (
                <option key={quarter} value={quarter} />
              ))}
            </datalist>
            <small className={styles.fieldHint}>
              Suggestions disponibles — saisie libre autorisée.
            </small>
          </label>
